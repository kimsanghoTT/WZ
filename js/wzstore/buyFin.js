...
document.addEventListener("DOMContentLoaded", () => {
  ...
  reviewBtn.addEventListener("click", (e) => {
    e.preventDefault();
    // 같은 폴더면 ↓
    window.location.href = "wzstore.html";
  });
});
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const sendMethod = sessionStorage.getItem("sendMethod") || "";
  const payMethod  = sessionStorage.getItem("payMethod")  || "";
  let totalAmt     = sessionStorage.getItem("totalAmount") || "";

  if (sendEl && sendMethod) sendEl.textContent = sendMethod;
...
saveCheckoutPayload();
sessionStorage.getItem('totalAmount');
document.querySelector('#payAfter .total p span')?.textContent